...
#### C-4: テキスト選択・クリップボード検証（§18.4）

- [ ] macOS: `.target` テキストを選択して Cmd+C → テキストエディタに貼り付けて内容を確認する
- [ ] Windows: Ctrl+C → メモ帳に貼り付けて内容を確認する
- [ ] iOS Safari: 長押しコピー → メモに貼り付けて内容を確認する（実機必須）
- [ ] Android Chrome: 長押しコピー → 任意アプリに貼り付けて内容を確認する（実機必須）
- [x] 結果を §18.5 のテンプレートの「Step 4」欄に記入する（事前自動検証 + 未実施項目を明記）
